...
class Downloader():
    ...
    def collection(self, user_name, extra_params):
        ...
        collection_data.append(_create_blank_collection(EXTRA_EXPANSIONS_GAME_ID, "😊无游戏扩展 (A-I)"))
...
def filter_duplicates(collection):
    seen = set()
    unique = [obj for obj in collection if obj["id"] not in seen and not seen.add(obj["id"])]
    return unique
